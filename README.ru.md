...
с помощью которой микроконтроллер может управлять нагрузкой посредством дискретного (ВКЛ./ВЫКЛ.)
и псевдоаналогового (ШИМ) сигналов, а также принимать команды по UART и по беспроводным сетям.

Далее по тексту название "Билли" может относиться как к самой программе, так и к устройству, которое её выполняет.
...
- **__.06.2024** - релиз версии 1.3 со следующими важными изменениями:
    - Добавлен функционал управления сигналом ШИМ, в связи с чем название проекта изменено
...
- Добавить в веб-интерфейс панель управления сигналом ШИМ.
...
### Нагрузка
Для управления нагрузкой Билли использует два пина - один для дискретного сигнала, один для сигнала ШИМ.
Один дополнительный пин использует дискретный сигнал для управления индикаторным светодиодом.

Помимо светодиодов, Билли может управлять такой типичной нагрузкой как:
- электромеханическим реле (с помощью драйвера на транзисторе или оптопаре) - посредством дискретного сигнала;
- электромотором (с помощью драйвера на КМОП-транзисторе) - посредством сигнала ШИМ.

Пины назначаются директивой `#define` в 

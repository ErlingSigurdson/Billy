...
Модули в основном не ссылаются друг на друга, вместо этого относящиеся к ним функции-врапперы комбинируются
в функциях `setup()`, `setip_WiFi()`. `setup_BTClassic()`, `loop()` и в функциях-обработчиках команд.
Благодаря этому упрощается допиливание скетча под конкретные нужды.

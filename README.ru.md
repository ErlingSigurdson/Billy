...
### Концепция
Билли - это написанная под фреймворк Ардуино программа (скетч) для микроконтроллеров ESP32 и ESP8266,
с помощью которой микроконтроллер может управлять нагрузкой посредством дискретного сигнала (ВКЛ./ВЫКЛ.)
...
(микроконтроллеру, модулю, плате разработки), которое её выполняет.
...
- **14.06.2024** - релиз версии 1.3 со следующими важными изменениями:
...
### Дальнейшие планы
1. Добавить в веб-интерфейс панель управления сигналом ШИМ.
2. Добавить поддержку Bluetooth Low Energy (BLE).
3. Добавить возможность переключения Wi-Fi между режимом станции и режимом точки доступа,
...
- при управлении сигналом ШИМ - электромотор с драйвером на КМОП-транзисторе;
- при управлении любым видом сигнала - светодиод (обычно используется для проверки работы устройства).

### Индикаторный светодиод
Билли использует один дополнительный вывод для дискретного управления индикаторным светодиодом,
сигнализирующим о статусе WiFi. Редкие мигания означают попытки подключиться к сети,
частые мигания - успешное подключение.

### Совмещение выводов
Следует избегать попыток управлять дискретным сигналом и сигналом ШИМ с одного и того же вывода.
Совмещать дискретное управление нагрузкой и индикаторным светодиодом допускается, но это будет означать,
...
Доступ в Интернет Билли получает через точку доступа, к которой он подключён в рамках сети Wi-Fi.
...
- в ответ на направляемый Билли запрос (например, строку `"UPD_REQ"`) направлять ему сообщение (тоже строку),
...
на следующий запрос Билли отправить ему команду `AT+DLOAD=ON`. Билли в роли TCP-клиента принимает команду
...
### Быстрый старт
Чтобы прошить, запустить, настроить и опробовать Билли, выполните следующие шаги:
...
- раскомментируйте директиву `#define INVERTED_DIGITAL_OUTPUT`, если ваша нагрузка включается при низком уровне
дискретного сигнала;
...
6. С помощью команды `AT+LOCALSSID=<значение>` введите SSID точки доступа сети Wi-Fi.
7. С помощью команды `AT+LOCALPSWD=<значение>` введите пароль точки доступа сети Wi-Fi.
8. С помощью команды `AT+LOCALPORT=<значение>` задайте номер порта, который Билли будет использовать как локальный
TCP-сервер.
9. Перезагрузите устройство или введите команду `AT+LOCALCONNRST`.
10. Убедитесь, что Билли подключился к локальной сети.
11. С помощью команд `AT+DLOAD=ON`, `AT+DLOAD=OFF`, `AT+DLOAD=TOGGLE` и `AT+ALOAD=<значение от 0 до 255>`,
передаваемых через UART и через консоль передачи данных по Wi-Fi, поуправляйте подключенной к соответствующим выводам
нагрузкой.
...
Чтобы опробовать функционал Bluetooth Classic:

1. С помощью команды `AT+BTCLASSICDEVNAME=<значение>` задайте имя устройства как ведомого устройства Bluetooth Classic
2. С помощью команды `AT+BTCLASSIC=ON` включите функционал Bluetooth.
3. С помощью команд `AT+DLOAD=ON`, `AT+DLOAD=OFF`, `AT+DLOAD=TOGGLE` и `AT+ALOAD=<значение от 0 до 255>`,
передаваемых через консоль передачи данных по Bluetooth, поуправляйте подключенной к соответствующим выводам
нагрузкой.
...
4. С помощью команды `AT+IOT=OFF` включите функционал IoT.
...
компиляцией конкатенируются средой разработки, либо с упаковкой в библиотеки), но я нахожу его удобным, поскольку
он позволяет определять порядок включений и делает структуру кода более прозрачной.
...
Модули в основном не ссылаются друг на друга, вместо этого относящиеся к ним функции-врапперы комбинируются в файлах
`Billy.ino`, благодаря чему упрощается допиливание скетча под конкретные нужды.
...
Из "голых" TCP-сообщений опытный пользователь может слепить практически что угодно, например, HTTP-запросы.

### Почему "Билли"?
В честь попугая-программиста, героя [мемных видео](https://www.youtube.com/watch?v=0MhVkKHYUAY&list=PLkdGijFCNuVmu35l6EJxdvsvf7xj4EQVf&index=21)
...
чтобы они имели вид `ATMYPSWD+DLOAD=TOGGLE`, таким образом фактически задав устройству пароль.

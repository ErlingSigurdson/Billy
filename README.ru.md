...
1. В файле `config_general.h` в значении директив `#define` укажите:
- будет ли использоваться Bluetooth Classic (`BT_CLASSIC_PROVIDED`, только для ESP32; для ESP8266 иррелевантно);
- пин управления нагрузкой (`LOAD_PIN`);
- пин управления индикаторным светодиодом (`WIFI_INDICATOR_LED_PIN`);
- режим работы нагрузки (`INVERTED_OUTPUT`).
...
Модули не ссылаются друг на друга, вместо этого относящиеся к ним функции-врапперы комбинируются в файле `.ino`, благодаря чему упрощается допиливание скетча под конкретные нуждыё

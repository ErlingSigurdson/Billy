...
### Концепция
Билли - это программа (скетч) для фреймворка Ардуино, написанная для микроконтроллеров (однокристальных систем)
ESP32 и ESP8266. С её помощью микроконтроллер может управлять нагрузкой посредством дискретного сигнала (ВКЛ./ВЫКЛ.)
и сигнала ШИМ, а также принимать команды по UART и по беспроводным сетям.

Управлять 7-сегментным 4-знаковым дисплеем с помощью двух последовательно соединённых микросхем 74HC595 _TODO_
...
- **25.06.2024** - релиз версии 1.3 со следующими важными изменениями:
    - добавлена поддержка управления сигналом ШИМ, в связи с чем название проекта
      изменено с "Релешка Билли" на просто "Билли";
...
- **28.06.2024** - релиз версии 1.3.1. В веб-интерфейс добавлена панель управления сигналом ШИМ.
- **29.07.2024** - релиз версии 1.3.2. Добавлена команда вывода информации
                   о загруженной версии скетча и о назначенных выводах.
- **17.06.2025** - релиз версии 1.4.0. Теперь в веб-интерфейсе для управления дискретным сигналом
                   используются кнопки, а не текстовое поле. Также исправлены многочисленные досадные
                   ошибки и реализованы многочисленные мелкие улучшения.
...
3. Заменить текстовое поле ввода значения, управляющего скважностью сигнала ШИМ, на ползунок,
   созданный с помощью фронтендового JavaScript.
...
- раскомментируйте директиву `#define BTCLASSIC_PROVIDED`, если ваш модуль ESP32 поддерживает Bluetooth Classic
и вы намерены его использовать. Игнорируйте для ESP8266.
...
8. С помощью команды `AT+LOCALPORT=<значение>` задайте номер порта, который будет использоваться локальным TCP-сервером
...
Чтобы опробовать функционал управления 7-сегментным 4-знаковым дисплеем с помощью двух последовательно соединённых
микросхем 74HC595:
1. _TODO_

NTP и соответствующие команды _TODO_
Команда установки текущего времени _TODO_
Коммуникация с RTC - часами реального времени (I2C/SPI) _TODO_
...
### Кастомизация
Добавить новую команду очень просто, достаточно выполнить следующие простые шаги:
- Добавьте вашу команду в список макросов в `config_cmd.h`.
- Объявите и определите функцию-обработчик для вашей команды в `cmd.h` и `cmd.cpp`.
- Вставьте макро вашей команды в качестве дополнительного (добавленного через запятую) значения, присваиваемого
переменной `cmd_list` в `Billy.ino`.
- Добавьте новую ветку (дополнителный оператор `case`) в раздел "Check fo valid commands" в `Billy.ino` и включите
в эту ветку вызов вашей функции-обработчика.

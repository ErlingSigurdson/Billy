...
uint32_t esp_tcp_billy::server_port_update(uint32_t port)
{
...
bool esp_tcp_billy::server_start()
{
    if (gp_TCP_local_server) {
...
bool esp_tcp_billy::server_get_client()
{
    if (gp_TCP_local_server) {
...
uint32_t esp_tcp_billy::server_read_line(char *buf, uint32_t str_max_len, uint32_t conn_timeout)
{
...
bool esp_tcp_billy::server_send_msg(const char *msg)
{
...
bool esp_tcp_billy::server_stop(uint32_t shutdown_downtime)
{
    if (gp_TCP_local_server) {
...
bool esp_tcp_billy::client_get_server(char *target_IP, uint32_t target_port)
{
...
void esp_tcp_billy::client_send_msg(const char *msg)
{
...
uint32_t esp_tcp_billy::client_read_line(char *buf, uint32_t str_max_len, uint32_t conn_timeout)
{
    // Connection timeout counter.
    uint64_t current_millis = millis();
    uint64_t previous_millis = current_millis;
...
void esp_tcp_billy::clients_disconnect(uint32_t shutdown_downtime)
{
    if (TCP_remote_client.connected()) {
        delay(shutdown_downtime);
